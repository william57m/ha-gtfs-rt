...
class PublicTransportData:
    ...
    def _update(self) -> None:
        ...
        try:
            # Try to run in a new event loop if we're not in one
            try:
                loop = asyncio.get_running_loop()
                # We're in an event loop, so we can't use asyncio.run
                # Create a task and let it run without awaiting
                asyncio.create_task(self._update_async())
            except RuntimeError:
                # No running event loop, we can use asyncio.run
                asyncio.run(self._update_async())
        except Exception as e:
            LoggerHelper.log_error([f"Failed to update transit data: {e}"], logger=_LOGGER)

    async def _update_async(self) -> None:
        """Async version of update method."""
        try:
            vehicle_positions = (
                await self._get_vehicle_positions() if self._vehicle_position_url else {}
            )
            await self._update_route_statuses(vehicle_positions)
        except GTFSFeedError as e:
            LoggerHelper.log_error([f"Error updating route statuses: {e}"], logger=_LOGGER)
        except Exception as e:
            LoggerHelper.log_error([f"Unexpected error in async update: {e}"], logger=_LOGGER)
